...
class RedisRetrievalChain(Core):
    ...
    def __ask_question(self, q, callback_fn: StreamingStdOutCallbackHandlerYield):
        # LLM Memory: Chatbot with history managed by LangChain
        redis_history = RedisChatMessageHistory(url=generate_redis_connection_string(REDIS_CFG["host"], REDIS_CFG["port"], REDIS_CFG["password"]),
                                                session_id=self.session_id,
                                                key_prefix='minipilot:history:',
                                                ttl=MINIPILOT_HISTORY_TIMEOUT)
        # LLM Memory: Limit the history length to MINIPILOT_HISTORY_LENGTH
        ...
        # LLM Memory: Helper method langchain
        ...
        if self.cfg.is_semantic_cache():
            ...
            if len(cached) > 0:
                ...
                # LLM Memory: The question is in the cache, but we want to save the conversation in the history too
                metadata = {}
        ...
        # Agentic memory: contstructing the QA prompts
        messages = [
        ...
        # Instantiating the LLM = OpenAI(streaming=True, callbacks=[callback_fn])
        ...
        1. Use the chat history and the new question to create a "standalone question". This is done so that this question can be passed into the retrieval step to fetch relevant documents.
           If only the new question was passed in, then relevant context may be lacking.
           If the whole conversation was passed into retrieval, there may be unnecessary information there that would distract from retrieval.
        ...
        note now you can rebuild this using https://js.langchain.com/v0.1/docs/use_cases/code_understanding/#chat
        https://python.langchain.com/v0.2/docs/versions/migrating_chains/conversation_retrieval_chain/
        """
        # The actual chat bot construction:
        ...
        try:
            ...
            # Semantic Cache: decide if the interaction should be cached
